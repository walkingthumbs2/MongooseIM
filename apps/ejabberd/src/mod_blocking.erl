...
process_iq_get(_, From, _To,
    #iq{xmlns = ?NS_BLOCKING, sub_el = #xmlel{name = <<"blocklist">>}},_) ->
...
process_iq_set(_, 
              ...
              #iq{ xmlns = ?NS_BLOCKING,
                   sub_el = #xmlel{name = SubElName, children = SubEls}
                 }) ->

#jid{luser = LUser, lserver = LServer} = From, Res = 
...
process_blocklist_block(LUser, LServer, JIDs) ->
    ...
    case process_blocklist_block(LUser, LServer, Filter, odbc) of
...
process_blocklist_block(LUser, LServer, Filter, mnesia) ->
  ...
  process_blocklist_block(LUser, LServer, Filter, odbc) ->
    F = fun () ->
      Default = case mod_privacy_odbc:sql_get_default_privacy_list_t(LUser) of
            {selected, [<<"name">>], []} ->
                Name = <<"Blocked contacts">>,
                mod_privacy_odbc:sql_add_privacy_list(LUser, Name),
                mod_privacy_odbc:sql_set_default_privacy_list(LUser,
                           Name),
                Name;
            {selected, [<<"name">>], [{Name}]} ->
              Name
      ...
      R = mod_privacy_odbc:sql_get_privacy_list_id_t(LUser, Default),
      {selected, [<<"id">>], [{ID}]} = R,
      case mod_privacy_odbc:sql_get_privacy_list_data_by_id_t(ID) of
        {selected,
         [<<"t">>, <<"value">>, <<"action">>, <<"ord">>,
          <<"match_all">>, <<"match_iq">>, <<"match_message">>,
          <<"match_presence_in">>, <<"match_presence_out">>], RItems = [_ | _]} ->
            List = lists:map(fun mod_privacy_odbc:raw_to_item/1, RItems);
        _ -> List = []
      end,
      NewList = Filter(List),
      NewRItems = lists:map(fun mod_privacy_odbc:item_to_raw/1,
                NewList),
...
process_blocklist_unblock_all(LUser, LServer) ->
    ...
    case process_blocklist_unblock_all(LUser, LServer, Filter,odbc) of
      {atomic, ok} -> {result, []};
      {atomic, {ok, Default, List}} ->
      UserList = make_userlist(Default, List),
...
process_blocklist_unblock_all(LUser, LServer, Filter,
            odbc) ->
    F = fun () ->
    case mod_privacy_odbc:sql_get_default_privacy_list_t(LUser)
        of
      {selected, [<<"name">>], []} -> ok;
      ...
      {selected,
       ...
       RItems = [_ | _]} ->
          List = lists:map(fun mod_privacy_odbc:raw_to_item/1,
               RItems),
          NewList = Filter(List),
          NewRItems = lists:map(fun mod_privacy_odbc:item_to_raw/1,
              NewList),
...
process_blocklist_unblock(LUser, LServer, JIDs) ->
    ...
    case process_blocklist_unblock(LUser, LServer, Filter,odbc)
  of
      {atomic, ok} -> {result, []};
      {atomic, {ok, Default, List}} ->
    UserList = make_userlist(Default, List),
...
%%
%%  At this point this method does not work. only odbc privacy lists are supported.
%%
...
broadcast_list_update(LUser, LServer, Name, UserList) ->
    ...
    Packet = #xmlel{  name = <<"broadcast">>,
                     children =  [{privacy_list,
                                  UserList,
                                   Name}]},
    ejabberd_router:route(JID, JID,Packet).

broadcast_blocklist_event(LUser, LServer, Event) ->
    JID = jlib:make_jid(LUser, LServer, <<"">>),
    Packet = #xmlel{name = <<"broadcast">>,
                             children = [{blocking, Event}]},
...
process_blocklist_get(LUser, LServer) ->
    ...
    case process_blocklist_get(LUser, LServer,odbc) of
      error ->
         {error, ?ERR_INTERNAL_SERVER_ERROR};
      List ->
...
process_blocklist_get(LUser, LServer, odbc) ->
    case catch
     mod_privacy_odbc:sql_get_default_privacy_list(LUser, LServer)
  of
      {selected, [<<"name">>], []} -> [];
      {selected, [<<"name">>], [{Default}]} ->
    case catch mod_privacy_odbc:sql_get_privacy_list_data(LUser,
                 LServer, Default)
        of
      {selected,
       [<<"t">>, <<"value">>, <<"action">>, <<"ord">>,
        ...
        <<"match_presence_in">>, <<"match_presence_out">>],
       RItems} ->
    lists:map(fun mod_privacy_odbc:raw_to_item/1, RItems);
      {'EXIT', _} -> error
    end;
      {'EXIT', _} -> error
    end.