    ...
    case NextState1 of
        ...
        normal_state ->
            {next_state, NextState1,
             StateData3#state{just_created = false}}
    end;
...
locked_state({route, From, ToNick,
    ...
    case xml:get_attr_s(<<"type">>, Attrs) =:= <<"unavailable">>
        ...
        true ->
            %% Will let the owner leave and destroy the room if it's not persistent
...
normal_state({route, From, <<>>,
             ...
             StateData) ->
    NewStateData = route_iq(#routed_iq{iq = jlib:iq_query_info(Packet),
                                       from = From,
                                       packet = Packet},
                            StateData),
...
normal_state({route, From, ToNick,
             ...
             StateData) ->
    Type = xml:get_attr_s(<<"type">>, Attrs),
    NewStateData = route_nick_message(
        #routed_nick_message{
...
handle_sync_event({change_state, NewStateData}, _From, StateName, _StateData) ->
    {reply, {ok, NewStateData}, StateName,
     NewStateData};
...
%%          {stop, Reason, NewStateData}
%%----------------------------------------------------------------------
handle_info({process_user_presence, From}, normal_state, StateData) ->
...
handle_info({process_user_message, From}, normal_state, StateData) ->
    RoomQueueEmpty = queue:is_empty(StateData#state.room_queue),
    ...
    NewState =
        case ?DICT:find(From, StateData#state.robots) of
            {ok, {Nick, Packet}} ->
                ...
                Err = jlib:make_error_reply(Packet, ?ERR_NOT_AUTHORIZED),
...
process_groupchat_message(From,
    ...
    Allowed = is_user_online(From, StateData) orelse
              is_user_allowed_message_nonparticipant(From, StateData),
    process_groupchat_message_if_allowed(Allowed, From, Packet, StateData).

process_groupchat_message_if_allowed(true, From, Packet, StateData) ->
    {_, Role} = get_participant_data(From, StateData),
    MayBroadcast = is_broadcast_messaging_allowed(Role, StateData),
    process_groupchat_broadcast_message(MayBroadcast, From, Packet, StateData);

process_groupchat_message_if_allowed(false, From,
...
is_broadcast_messaging_allowed(moderator, _StateData) ->
    true;
is_broadcast_messaging_allowed(participant, _StateData) ->
    true;
is_broadcast_messaging_allowed(_Role, StateData) ->
    ((StateData#state.config)#config.moderated == false).

process_groupchat_broadcast_message(true, From,
                                    {xmlelement, _, Attrs, _} = Packet,
                                    StateData) ->
                ...
                case can_change_subject(Role, StateData) of
                    true ->
                        ...
                        case (NSD#state.config)#config.persistent of
                            true ->
                                mod_muc:store_room(NSD#state.host,
                                                   NSD#state.room,
                                                   make_opts(NSD));
                            _ ->
                                ok
                    ...
                    true ->
                        ?ERRT_FORBIDDEN(Lang,
                                        <<"Only moderators and participants ",
                                          "are allowed to change the subject ",
                                          "in this room">>);
                    _ ->
                        ?ERRT_FORBIDDEN(Lang,
                                        <<"Only moderators are allowed to ",
                                          "change the subject in this room">>)
                end,
            ejabberd_router:route(
...
process_groupchat_broadcast_message(false, From,
                                    {xmlelement, _, Attrs, _} = Packet,
                                    StateData) ->
    Lang = xml:get_attr_s(<<"xml:lang">>, Attrs),
...
process_presence(From, Nick, {xmlelement, <<"presence">>, Attrs, _Els} = Packet,
                 StateData) ->
    Type = xml:get_attr_s(<<"type">>, Attrs),
    StateData1 = process_presence_by_type(Type, From, Nick, Packet, StateData),
    case is_room_temporary(StateData1) andalso is_room_empty(StateData1) of
        true ->
...
is_room_temporary(StateData) ->
    not (StateData#state.config)#config.persistent.

is_room_empty(StateData) ->
    ?DICT:to_list(StateData#state.users) == [].

process_presence_by_type(<<"unavailable">>, From, _Nick, Packet, StateData) ->
    case is_user_online(From, StateData) of
        true ->
            ...
            NewState = add_user_unavailable_presence(From, NewPacket,
                                                     StateData),
...
process_presence_by_type(<<"error">>, From, _Nick,
                         {xmlelement, _, Attrs, _} = Packet,
                         StateData) ->
    Lang = xml:get_attr_s(<<"xml:lang">>, Attrs),
    case is_user_online(From, StateData) of
...
process_presence_by_type(<<>>, From, Nick, Packet, StateData) ->
    IsOnline = is_user_online(From, StateData),
    process_presence_from_online_user(IsOnline, From, Nick, Packet, StateData);

process_presence_by_type(_Type, _From, _Nick, _Packet, StateData) ->
    StateData.

process_presence_from_online_user(true, From, Nick,
                                  {_,_,Attrs,_} = Packet, StateData) ->
    case is_nick_change(From, Nick, StateData) of
        true ->
            AllowNickchange =
                (StateData#state.config)#config.allow_visitor_nickchange,
            Status = {does_nick_exist(Nick, StateData),
                      mod_muc:can_use_nick(StateData#state.host,
                                           From, Nick),
                      {AllowNickchange,
                       is_visitor(From, StateData)}},
            Lang = xml:get_attr_s(<<"xml:lang">>, Attrs),
            maybe_change_nick(Status, Lang, From, Nick, Packet, StateData);
        _NotNickChange ->
...
%% (user wants to enter the room) and that the user
%% is not already online
process_presence_from_online_user(false, From, Nick,
                                  {xmlelement, _, Attrs, _} = Packet,
                                  StateData) ->
...
handle_new_user(From, Nick = <<>>, _Packet, StateData, Attrs) ->
    Lang = xml:get_attr_s(<<"xml:lang">>, Attrs),
    ErrText = <<"No nickname">>,
    Error =jlib:make_error_reply({xmlelement,<<"presence">>, [], []},
...
maybe_change_nick({_, _, {false, true}}, Lang, From, Nick, Packet, StateData) ->
    ErrText = <<"Visitors are not allowed to change ",
                "their nicknames in this room">>,
...
maybe_change_nick({true, _, _}, Lang, From, Nick, Packet, StateData) ->
...
maybe_change_nick({_, false, _}, Lang, From, Nick, Packet, StateData) ->
...
maybe_change_nick(_, _Lang, From, Nick, _Packet, StateData) ->
...
stanzaid_pack(OriginalId, Resource) ->
    <<"berd">> ++
    base64:encode_to_string(<<"ejab\0">> ++ OriginalId ++ <<"\0">> ++ Resource).
...
binary_to_role(<<"moderator">>)   -> moderator;
binary_to_role(<<"participant">>) -> participant;
binary_to_role(<<"visitor">>)     -> visitor;
binary_to_role(<<"none">>)        -> none.

role_to_binary(moderator)   -> <<"moderator">>;
role_to_binary(participant) -> <<"participant">>;
role_to_binary(visitor)     -> <<"visitor">>;
role_to_binary(none)        -> <<"none">>.

affiliation_to_binary(owner)   -> <<"owner">>;
affiliation_to_binary(admin)   -> <<"admin">>;
affiliation_to_binary(member)  -> <<"member">>;
affiliation_to_binary(outcast) -> <<"outcast">>;
affiliation_to_binary(none)    -> <<"none">>.

binary_to_affiliation(<<"owner">>)   -> owner;
binary_to_affiliation(<<"admin">>)   -> admin;
binary_to_affiliation(<<"member">>)  -> member;
binary_to_affiliation(<<"outcast">>) -> outcast;
binary_to_affiliation(<<"none">>)    -> none.
...
get_error_condition2(Packet) ->
    ...
    [Condition] =
        [Name || {xmlelement, Name, [{<<"xmlns">>, ?NS_STANZAS}], []} <- EEls],
    {condition, Condition}.
...
expulse_participant(Packet, From, StateData, Reason1) ->
    ...
    NewState =
        add_user_unavailable_presence(From,
                                      {xmlelement, <<"presence">>,
                                       [{<<"type">>, <<"unavailable">>}],
                                       [{xmlelement, <<"status">>, [],
                                         [{xmlcdata, Reason2}]}]},
...
get_affiliation(JID, StateData) ->
    {_, _, AccessAdmin, _} = StateData#state.access,

    Res = case acl:match_rule(StateData#state.server_host, AccessAdmin, JID) of
              allow ->
                  owner;
              _ ->
                  LJID  = jlib:jid_tolower(JID),
                  ...
                  %% ?DICT:find(...) -> {ok, Found} | term() so we can
                  %% pass the successful finding along
                  Find = fun(_, {ok,_} = Acc) ->
                                Acc;
                            (Jid, _) ->
            ...
            false ->
                ...
                of
                    true ->
                        ...
                        StateData#state{activity = treap:insert(Key, Priority,
                                                                UserActivity,
                                                                Activity)};
                    false ->
                        Priority = {0, 0},
...
strip_status({xmlelement, <<"presence">>, Attrs, Els}) ->
    FEls = lists:filter(fun({xmlelement, <<"status">>, _Attrs1, _Els1}) ->
                                false;
                           (_) ->
                                true
                        end, Els),
...
add_user_unavailable_presence(JID, Presence, StateData) ->
    LJID = jlib:jid_tolower(JID),
    FPresence = filter_presence(Presence),
    Users = ?DICT:update(LJID,
                         fun(#user{} = User) ->
...
does_nick_exist(Nick, StateData) ->
...
can_add_new_user(Affiliation, ServiceAffiliation, From, StateData) ->
    MaxUsers = get_max_users(StateData),
    MaxAdminUsers = MaxUsers + get_max_users_admin_threshold(StateData),
    NUsers = dict:fold(fun(_, _, Acc) -> Acc + 1 end, 0,
                       StateData#state.users),
    NConferences = tab_count_user(From),
    MaxConferences = gen_mod:get_module_opt(StateData#state.server_host,
...
add_new_user(From, Nick, {xmlelement, _, Attrs, Els} = Packet, StateData) ->
    ...
    case {can_add_new_user(Affiliation, ServiceAffiliation, From, StateData),
          does_nick_exist(Nick, StateData),
    ...
    of
        ...
        {_, _, false, _} ->
            ...
            StateData;
        {_, _, _, Role} ->
            maybe_add(check_password(ServiceAffiliation, Affiliation,
                                     Els, From, StateData),
                      Role, From, Nick, Packet, StateData)
    end.

maybe_add(true = _PasswordAccepted, Role, From, Nick,
          {xmlelement, _, Attrs, Els} = Packet, StateData) ->
    Lang = xml:get_attr_s(<<"xml:lang">>, Attrs),
    NewState =
...
maybe_add(nopass, _Role, From, Nick,
          {xmlelement, _, Attrs, _} = Packet, StateData) ->
    Lang = xml:get_attr_s(<<"xml:lang">>, Attrs),
...
maybe_add(captcha_required, _Role, From, Nick,
          {xmlelement, _, Attrs, _} = Packet, StateData) ->
    Lang = xml:get_attr_s(<<"xml:lang">>, Attrs),
    ...
    case ejabberd_captcha:create_captcha(SID, RoomJID, To, Lang,
                                         Limiter, From) of
        {ok, ID, CaptchaEls} ->
            MsgPkt = {xmlelement, <<"message">>, [{<<"id">>, ID}], CaptchaEls},
            Robots = ?DICT:store(From, {Nick, Packet}, StateData#state.robots),
...
count_stanza_shift(Nick, Els, StateData) ->
    ...
    Shift1 = case Seconds of
                 false ->
                     0;
                 _ ->
                     UTCNow = calendar:now_to_universal_time(now()),
                     Sec = calendar:datetime_to_gregorian_seconds(UTCNow)
                            - Seconds,
...
count_seconds_shift(Seconds, HistoryList) ->
    Shift = fun({_Nick, _Packet, _HaveSubject, TimeStamp, _Size}) ->
    ...
    lists:sum(lists:map(Shift, HistoryList)).
...
send_new_presence(NJID, Reason, StateData) ->
    ...
    BinAffiliation = affiliation_to_binary(Affiliation),
    BinRole = role_to_binary(Role),
    F = fun({_LJID, Info}) ->
    ...
    lists:foreach(F, ?DICT:to_list(StateData#state.users)).
...
send_invitation(From, To, Reason, StateData) ->
    ...
    ejabberd_router:route(StateData#state.jid, To,
        jlib:make_invitation(jlib:jid_replace_resource(From, <<>>),
                             Password, Reason)).
...
send_history(JID, Shift, StateData) ->
    lists:foldl(fun({Nick, Packet, HaveSubject, _TimeStamp, _Size}, B) ->
                    ejabberd_router:route(
                        jlib:jid_replace_resource(StateData#state.jid, Nick),
                        JID, Packet),
                    B or HaveSubject
                ...
                false,
                lists:nthtail(Shift, lqueue_to_list(StateData#state.history))).
            ...
            case xml:get_tag_attr(<<"role">>, Item) of
                false ->
                    case xml:get_tag_attr(<<"affiliation">>, Item) of
                        false ->
                            {error, ?ERR_BAD_REQUEST};
                        {value, BinAffiliation} ->
                            try binary_to_affiliation(BinAffiliation) of
                                Affiliation when FAffiliation == owner;
                                                 FAffiliation == admin ->
                                    Items = items_with_affiliation(Affiliation,
                                                                   StateData),
...
items_with_affiliation(BinAffiliation, StateData) ->
    lists:map(
     ...
     ({JID, Affiliation}) ->
        ...
        {<<"jid">>, jlib:jid_to_binary(JID)}],
           []}
      end, search_affiliation(BinAffiliation, StateData)).
    ...
    case find_changed_items(UJID, UAffiliation, URole, Items, Lang, StateData, []) of
        {result, Res} ->
            ?INFO_MSG("Processing MUC admin query from ~s in room ~s:~n ~p",
                      [jlib:jid_to_binary(UJID),
                       jlib:jid_to_binary(StateData#state.jid),
                       Res]),
            NSD = lists:foldl(fun(E, SD) ->
                                  try process_admin_item(E, UJID, SD) of
                                      NSD ->
                                          NSD
                                  catch error:Reason ->
...
process_admin_item({JID, affiliation, owner, _}, _UJID, SD)
        when (JID#jid.luser == <<>>) ->
    %% If the provided JID does not have username,
    %% forget the affiliation completely
    SD;

process_admin_item({JID, role, none, Reason}, _UJID, SD) ->
    catch send_kickban_presence(JID, Reason, <<"307">>, SD),
    set_role(JID, none, SD);

process_admin_item({JID, affiliation, none, Reason}, _UJID, SD) ->
...
process_admin_item({JID, affiliation, outcast, Reason}, _UJID, SD) ->
    catch send_kickban_presence(JID, Reason, <<"301">>, outcast, SD),
    set_affiliation_and_reason(JID, outcast, Reason,
                               set_role(JID, none, SD));

process_admin_item({JID, affiliation, A, Reason}, _UJID, SD)
...
process_admin_item({JID, affiliation, member, Reason}, UJID, SD) ->
    case (SD#state.config)#config.members_only of
        true ->
            send_invitation(UJID, JID, Reason, SD);
        _ ->
            ok
    end,
...
process_admin_item({JID, role, Role, Reason}, _UJID, SD) ->
                        ...
                        {value, BinAffiliation} ->
                            try binary_to_affiliation(BinAffiliation) of
                                Affiliation ->
                                    ...
                                    CanChangeRA =
                                    case can_change_ra(UAffiliation, URole,
                                                       TAffiliation, TRole,
                ...
                {value, BinRole} ->
                    try binary_to_role(BinRole) of
                        Role ->
                            ServiceAf = get_service_affiliation(JID, StateData),
                            CanChangeRA =
                                case can_change_ra(UAffiliation, URole, TAffiliation,
                                                   TRole, role, Role, ServiceAf)
                                of
                                    nothing ->
                                        nothing;
                                    true ->
                            ...
                            case CanChangeRA of
                                ...
                                true ->
                                    find_changed_items(UJID, UAffiliation, URole,
                                                       Items, Lang, StateData,
                                                       [{JID, role, Role,
                                                         xml:get_path_s(Item, [{elem, <<"reason">>}, cdata])} | Res]);
                                _ ->
                                    {error, ?ERR_NOT_ALLOWED}
                            end
                    catch error:_ ->
...
send_kickban_presence1(UJID, Reason, Code, Affiliation, StateData) ->
    ...
    BannedJID = jlib:jid_to_binary(RealJID),
    lists:foreach(
      fun({_LJID, Info}) ->
                        ...
                        {?NS_XDATA, <<"submit">>} ->
                            case Els1 of
                                ...
                                _ ->
                                    ...
                                    case can_change_config(XEl, From, StateData) of
                                        true ->
...
can_change_config(XEl, From, StateData) ->
    is_log_change_allowed(XEl, StateData, From)
    andalso
    is_persistency_change_allowed(XEl, StateData, From)
    andalso
    is_room_name_desc_limits_change_allowed(XEl, StateData)
    andalso
...
is_log_change_allowed(XEl, StateData, From) ->
    case lists:keymember(<<"muc#roomconfig_enablelogging">>, 1,
        ...
        false ->
            true;
        true ->
            allow == mod_muc_log:check_access_log(StateData#state.server_host,
                                                  From)
    end.
...
is_persistency_change_allowed(XEl, StateData, From) ->
    case lists:keymember(<<"muc#roomconfig_persistentroom">>, 1,
        ...
        false ->
            true;
        true ->
            {_, _, _, AccessPersistent} = StateData#state.access,
            allow == acl:match_rule(StateData#state.server_host,
...
get_default_room_maxusers(RoomState) ->
    DefRoomOpts = gen_mod:get_module_opt(RoomState#state.server_host,
...
get_config(Lang, StateData, From) ->
    ...
    Config = StateData#state.config,
    {MaxUsersRoomInteger, MaxUsersRoomBinary} =
    ...
    IntegerOption = fun(N) ->
                        {xmlelement, <<"option">>,
                         [{<<"label">>, integer_to_binary(N)}],
                         [{xmlelement, <<"value">>,
                           [],
                           [{xmlcdata, integer_to_binary(N)}]}]}
                    end,
    Res =
        [{xmlelement, <<"title">>, [],
        ...
        [?BOOLXFIELD(<<"Make room public searchable">>,
         ...
         {xmlelement, <<"field">>,
          ...
          [{xmlelement, <<"value">>, [], [{xmlcdata, MaxUsersRoomBinary}]}] ++
          if
          ...
          [ IntegerOption(N) || N <- lists:usort([ServiceMaxUsers,
                                                  DefaultRoomMaxUsers,
                                                  MaxUsersRoomInteger
...
%%     end ++
    ...
    {result,
     [{xmlelement, <<"instructions">>, [],
       [{xmlcdata,
         translate:translate(Lang, <<"You need an x:data capable client ",
                                     "to configure room">>)}]},
        {xmlelement, <<"x">>,
         [{<<"xmlns">>, ?NS_XDATA},
          {<<"type">>, <<"form">>}],
                ...
                #config{} = Config ->
                    ...
                    Type = case {(StateData#state.config)#config.logging,
                           ...
                           of
                               ...
                               {_, _} ->
                                   case {(StateData#state.config)#config.anonymous,
...
set_opts([{Opt, Val} | Opts], StateData) ->
    NSD = case Opt of
              title ->
                  StateData#state{config = (StateData#state.config)#config{title = Val}};
              description ->
                  StateData#state{config = (StateData#state.config)#config{description = Val}};
              allow_change_subj ->
                  StateData#state{config = (StateData#state.config)#config{allow_change_subj = Val}};
              allow_query_users ->
                  StateData#state{config = (StateData#state.config)#config{allow_query_users = Val}};
              allow_private_messages ->
                  StateData#state{config = (StateData#state.config)#config{allow_private_messages = Val}};
              allow_visitor_nickchange ->
                  StateData#state{config = (StateData#state.config)#config{allow_visitor_nickchange = Val}};
              allow_visitor_status ->
                  StateData#state{config = (StateData#state.config)#config{allow_visitor_status = Val}};
              public ->
                  StateData#state{config = (StateData#state.config)#config{public = Val}};
              public_list ->
                  StateData#state{config = (StateData#state.config)#config{public_list = Val}};
              persistent ->
                  StateData#state{config = (StateData#state.config)#config{persistent = Val}};
              moderated ->
                  StateData#state{config = (StateData#state.config)#config{moderated = Val}};
              members_by_default ->
                  StateData#state{config = (StateData#state.config)#config{members_by_default = Val}};
              members_only ->
                  StateData#state{config = (StateData#state.config)#config{members_only = Val}};
              allow_user_invites ->
                  StateData#state{config = (StateData#state.config)#config{allow_user_invites = Val}};
              password_protected ->
                  StateData#state{config = (StateData#state.config)#config{password_protected = Val}};
              captcha_protected ->
                  StateData#state{config = (StateData#state.config)#config{captcha_protected = Val}};
              password ->
                  StateData#state{config = (StateData#state.config)#config{password = Val}};
              anonymous ->
                  StateData#state{config = (StateData#state.config)#config{anonymous = Val}};
              logging ->
                  StateData#state{config = (StateData#state.config)#config{logging = Val}};
              captcha_whitelist ->
                  StateData#state{config = (StateData#state.config)#config{captcha_whitelist = ?SETS:from_list(Val)}};
              max_users ->
                  ServiceMaxUsers = get_service_max_users(StateData),
                  MaxUsers = if
                  ...
                  StateData#state{config = (StateData#state.config)#config{max_users = MaxUsers}};
              affiliations ->
...
get_roomdesc_reply(JID, StateData, Tail) ->
    ...
    if
        (StateData#state.config)#config.public or IsOccupantOrAdmin ->
            if
                (StateData#state.config)#config.public_list or IsOccupantOrAdmin ->
            ...
            case get_role(From, StateData) of
                moderator ->
                    case GetField(<<"muc#request_allow">>) of
                        <<"true">> ->
                            case GetField(<<"muc#roomnick">>) of
                                false ->
                                    {error, ?ERR_BAD_REQUEST};
                                RoomNick ->
                                    {role, BinRole, RoomNick}
                            end;
                         _ ->
                            ok
                    end;
...
check_invitation(From, Els, Lang, StateData) ->
    ...
    InviteEl = case xml:remove_cdata(Els) of
                   [{xmlelement, <<"x">>, _Attrs1, Els1} = XEl] ->
                       ...
                       case xml:remove_cdata(Els1) of
                           [{xmlelement, <<"invite">>, _, _} = InviteEl1] ->
                               InviteEl1;
                           _ ->
                               throw({error, ?ERR_BAD_REQUEST})
                       end;
    ...
    JID = case jlib:binary_to_jid(xml:get_tag_attr_s(<<"to">>, InviteEl)) of
              error ->
    ...
    case CanInvite of
        ...
        true ->
            Reason =
                xml:get_path_s(InviteEl, [{elem, <<"reason">>}, cdata]),
            ContinueEl =
            ...
            Body =
                {xmlelement, <<"body">>, [],
                 [{xmlcdata,
                   <<IMessage/binary, IPassword/binary, IReason/binary>>}]},
            Msg =
                {xmlelement, <<"message">>,
                 [{<<"type">>, <<"normal">>}],
                 [{xmlelement, <<"x">>,
                  ...
                  {xmlelement, <<"x">>,
                   [{<<"xmlns">>, ?NS_XCONFERENCE},
                    {<<"jid">>,
                     jlib:jid_to_binary({StateData#state.room, StateData#state.host, <<>>})}],
                   [{xmlcdata, Reason}]},
                  Body]},
...
add_to_log(Type, Data, StateData)
    ...
    mod_muc_log:add_to_log(StateData#state.server_host, roomconfig_change,
                           Data, StateData#state.jid,
                           make_opts(StateData));
add_to_log(Type, Data, StateData) ->
    case (StateData#state.config)#config.logging of
        true ->
            mod_muc_log:add_to_log(StateData#state.server_host, Type,
                                   Data, StateData#state.jid,
                                   make_opts(StateData));
        false ->
            ok
...
route_message(#routed_message{allowed = true, type = <<"groupchat">>,
                              from = From, packet = Packet, lang = Lang},
              StateData) ->
    Activity = get_user_activity(From, StateData),
    Now = now_to_usec(now()),
    MinMessageInterval =
        ...
        MessageShaperInterval == 0 ->
            ...
            if
                RoomShaperInterval == 0, RoomQueueEmpty ->
                    NewActivity =
                        Activity#activity{message_time = Now,
...
route_message(#routed_message{allowed = true, type = <<"error">>, from = From,
                              packet = Packet, lang = Lang},
                              StateData) ->
    case is_user_online(From, StateData) of
        true ->
            ErrorText = <<"This participant is kicked from the room ",
                          "because he sent an error message">>,
            expulse_participant(Packet, From, StateData,
                                translate:translate(Lang, ErrorText));
        _ ->
            StateData
...
route_message(#routed_message{allowed = true, type = <<"chat">>, from = From,
                              packet = Packet, lang = Lang},
              StateData) ->
...
route_message(#routed_message{allowed = true, type = Type, from = From,
                              packet = {xmlelement, <<"message">>, _Attrs, Els} = Packet,
                              lang = Lang},
              StateData)
          when (Type == <<>> orelse Type == <<"normal">>) ->
...
route_voice_approval({role, RoleName, Nick}, From, Packet, Lang, StateData) ->
    ...
    case process_admin_items_set(From, Els, Lang, StateData) of
        {result, _Res, SD1} ->
            SD1;
        {error, Error} ->
            ejabberd_router:route(StateData#state.jid, From,
...
route_nick_message(#routed_nick_message{decide = continue_delivery,
                                        allow_pm = true,
                                        online = true, packet = Packet,
                                        from = From, type = <<"groupchat">>,
                                        lang = Lang, nick = ToNick},
                   StateData) ->
    ErrText = <<"It is not allowed to send private messages ",
                "of type groupchat">>,
    Err = jlib:make_error_reply(Packet, ?ERRT_BAD_REQUEST(Lang, ErrText)),
    ejabberd_router:route(
...
route_nick_iq(#routed_nick_iq{allow_query = true, online = {true, _, _},
                              jid = false, iq = reply}, _StateData) ->
    ok;

route_nick_iq(#routed_nick_iq{allow_query = true, online = {true, _, _},
                              jid = false, packet = Packet, lang = Lang,
                              from = From, nick = ToNick}, StateData) ->
    ErrText = <<"Recipient is not in the conference room">>,
    Err = jlib:make_error_reply(
...
route_nick_iq(#routed_nick_iq{allow_query = true, online = {true, NewId, FromFull},
                              jid = ToJID, packet = Packet, stanza = StanzaId},
              StateData) ->
